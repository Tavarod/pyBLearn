...
color_numeros_bg = "#7D2181"      # Púrpura para el fondo de botones numericos
...
def pulsar_tecla(tecla_presionada):
    """
    Añade el caracter de la tecla presionada a la expresión en el visor
    Si se acaba de mostrar un resultado, inicia una nueva expresión,
    a menos que se presione un operador para continuar el calculo.
    """
...
def evaluar_expresion():
    """
    evalua la expresión matemática actual en el visor y muestra el resultado.
    maneja errores comunes como división por cero o sintaxis incorrecta.
    """
    global expresion, resultado_mostrado
    if not expresion:
        return
    #remplaza los simbolos por lo operadores arimeticos
    try:
...
for texto_btn, fila_btn, col_btn in botones_config:
    ...
    boton = tk.Button(
        ...
        activebackground=color_presionado_bg, # Color de fondo cuando esta presionado
        activeforeground=color_texto_presionado, # Color del texto cuando esta presionado
